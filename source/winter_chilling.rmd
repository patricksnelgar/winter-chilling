...
start_date <- "2022-01-01T08:00:00"
end_date <- "2022-08-24T08:00:00"
...
tpk_data_check %>% 
	...
	filter(StopDate < ymd_hm("2022-08-23 08:00")) %>% 
	rename(Temperature_old = 2, 
		   Temperature_api_download = 3) %>%
	mutate(StationID = "KER", 
		   Temperature_old = format(Temperature_old, digits = 4, nsmall = 2), 
		   Temperature_api_download = format(Temperature_api_download, digits = 4, nsmall = 2)) %>% 
	filter(!Temperature_old == Temperature_api_download 
		   & !is.na(Temperature_api_download)) %>% 
	select(StationID, StopDate, Temperature_old, Temperature_api_download) %>% 
	write_csv(here("output/ker_data_check_aug_2022.csv"))
