...
june_rainfall_summary %>%
	...
	mutate(smaller_than = case_when(rainfall >= first(rainfall) & !(year == current_year) ~ 1, 
									...
									TRUE ~ 0)) %>% 
	ggplot() +
		geom_bar(aes(factor(year), rainfall, fill = factor(smaller_than)), stat = "identity") +
...
# plot it as a cumulative line like chilling

june_breaks <- 
	june_rainfall %>% 
	filter(StationID == "TPK", Year == 2021) %$%
	pretty(x = DayOfYear, 30)

june_ticks <- 
	format(as.Date(june_breaks -1, origin), "%b-%d")

# THIS BREAKS IF THERE ARE NA's in rainfall
june_rainfall %>% 
	filter(StationID == "TPK" & Year >= current_year - 10) %>% 
	group_by(Year) %>% 
	mutate(RainTotal = ifelse(!is.na(RainTotal), RainTotal, 0),
		   cumulative_rainfall = cumsum(RainTotal),
		   DayOfJune = format(StopDate, "%d %H:%M")) %>% 
	select(StopDate, DayOfYear, Year, RainTotal, cumulative_rainfall) %>% 
	ggplot() +
		geom_line(aes(DayOfYear, cumulative_rainfall, colour = factor(Year), size = factor(Year))) +
		scale_x_continuous(breaks = june_breaks, labels = june_ticks) +
		scale_y_continuous(limits = c(0, 400)) +
		scale_size_manual(values = c(rep(1, 10), 2)) +
		guides(size = "none") +
		theme_bw()
...
seven_day_snapshot <- 
	...
	summarise(diff_chill = last(cumulative_chill) - first(cumulative_chill),
			  ...
			  average_temp = mean(TemperatureMean),
			  rainfall = sum(RainTotal))
