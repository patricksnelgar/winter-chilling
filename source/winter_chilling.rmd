...
by_site %>% 
	...
	ggplot() +
		...
		scale_color_manual(values = c("red", "blue", "gold","darkcyan", "orange", "purple",
						   ...
						   name = "Year") + 
		scale_size_manual(values = c(rep(0.5, 10), 1)) +
		scale_y_continuous(breaks = seq(0, 1500, 100), limits = c(0, 1200)) +
		scale_x_continuous(breaks = doy_ticks, labels = month_ticks) +
...
met_data %>% 
	filter(StationID == "TPK") %>% 
	group_by(Year) %>% 
	summarise(total_rainfall = sum(RainTotal, na.rm = TRUE)) %>% 
...
month_ticks <- 
	by_site %>% 
	filter(StationID == "TPK" & DayOfYear %in% yday(c("2022-05-01", "2022-06-01", "2022-07-01", "2022-08-01"))) %>% 
...
# need to change this back once 2017 has lapsed to keep colours aligned between sites.
by_site %>% 
	filter(StationID == "TPK" & (Month > 4 & Month < 8) & Year >= oldest_year & !Year == 2017) %>%
		ggplot() +
			geom_line(aes(DayOfYear, ChillingHoursCumulative, colour = factor(Year), size = factor(Year))) + 
			scale_size_manual(values = c(rep(0.5, 8),1, 1)) + 
			scale_color_manual(values = c("red", "blue", "gold","darkcyan", "orange",
...
ggsave(here("output/Te Puke winter chill for HC alts report 2023.png"), width = 8, height = 8)

by_site %>% 
	filter(StationID == "KER" & (Month > 4 & Month < 8) & Year >= oldest_year) %>%
		ggplot() +
			geom_line(aes(DayOfYear, ChillingHoursCumulative, colour = factor(Year), size = factor(Year))) + 
			scale_size_manual(values = c(rep(0.5, 9), 1)) + 
			scale_color_manual(values = c("red", "blue", "gold","darkcyan", "orange",
...
ggsave(here("output/Kerikeri winter chill for HC alts report 2023.png"), width = 8, height = 8)

historical <- 
	by_site %>%
	mutate(doy = yday(StopDate)) %>%
	filter(StationID %in% c("TPK", "KER")
		   & year(StopDate) >= current_year - 5 
		   & !year(StopDate) == current_year 
		   & !(StationID == "TPK" & year(StopDate) == 2017) 
		   & doy >= 121 & doy <= 213 
...
by_site %>%
	filter(StationID %in% c("TPK", "KER")
		   & Year == 2022 
		   & DayOfYear >= 121 & DayOfYear <= 213) %>%
	ggplot() +
		geom_line(aes(DayOfYear, ChillingHoursCumulative, colour = StationID)) +
		geom_line(aes(doy, historical_chill_hours, colour = "5 year mean"), data = five_year_chill) + 
		facet_wrap(~StationID, labeller = labeller(StationID = c("KER" = "Kerikeri", "TPK" = "Te Puke"))) + 
		scale_y_continuous(breaks = seq(0, 600, 50), limits = c(0, 600)) +
		scale_x_continuous(breaks = month_ticks, labels = month_labels) +
		scale_colour_manual(values = c("5 year mean" = "black", "TPK" = chill_colours[5], "KER" = chill_colours[2]), 
							labels = c("KER" = "Kerikeri", "TPK" = "Te Puke")) +
		labs(colour = "Site", x = "", y = "Cumulative chilling hours (<7°C)") +
		ggtitle(paste("Cumulative chilling hours vs 5 year mean - 2022")) +
		theme_bw() +
...
ggsave(here(paste0("output/Kerikeri and Te Puke 2022 winter chill vs 5 year mean.png")), width = 10, height = 6)
