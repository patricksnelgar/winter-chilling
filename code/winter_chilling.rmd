...
by_site %>% 
	filter(StationID == "TPK" & (Month > 4 & Month < 10) & Year >= oldest_year & !Year == 2017) %>%
		ggplot() +
			...
			#geom_vline(aes(xintercept = yday(ymd("2021-07-04"))))+
...
ker_interactive <- ggplotly(ker_chill)

saveWidget(ker_interactive, here("output/Winter chilling - Kerikeri 2021 - interactive.html"), selfcontained = TRUE)
...
by_site %>% 
	...
	ggplot() +
		geom_line(aes(DayOfYear, ChillingHoursCumulative, colour = "sub7C"))+
		geom_line(aes(DayOfYear, alt_cumulative, colour = "sub7.2C")) +
		scale_colour_manual(values = c("sub7C" = "black", "sub7.2C" = "skyblue")) + 
		scale_x_continuous(breaks = pretty_breaks(15), limits = c(120, 275)) +
		ylim(0, 500) + 
		theme_bw()
	...
	filter(StationID == "KER" & Year == 2021) %>% 
	slice(n()) %>% 
	select(ChillingHoursCumulative, alt_cumulative)
